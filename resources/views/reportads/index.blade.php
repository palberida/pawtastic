            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="mb-4 w-full">
                        <form method="GET" action="{{ route('report-ads.index') }}" class="flex flex-wrap items-center gap-4 w-full">
                    ...
                    <table class="w-full min-w-full divide-y divide-gray-200 mt-4 text-sm">
                        ...
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Cantidad</th>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Comision</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($ads as $ad)
                                <tr>
                                    <td class="px-4 py-4 whitespace-nowrap">{{ $ad->id }}</td>
                                    <td class="py-1 px-2 border-b">{{ number_format($ad->ad_costo,2) }}</td>
                                    <td class="px-4 py-4 whitespace-nowrap">
                                    <table class="min-w-full border border-gray-200 rounded-md text-sm">
                                        <tbody>
                                            @foreach(explode(',', $ad->product_descriptions ?? '') as $product)
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap">{{ $ad->total_cantidad }}</td>
                                    <td class="py-1 px-2 border-b">{{ number_format($ad->total_ventas,2) }}</td>
                                    <td class="py-1 px-2 border-b">{{ number_format($ad->total_costo,2) }}</td>
                                    <td class="py-1 px-2 border-b">{{ number_format($ad->total_comision,2) }}</td>
                                    <td class="py-1 px-2 border-b">{{ number_format($ad->total_ventas - $ad->ad_costo - $ad->total_costo - $ad->total_comision,2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
