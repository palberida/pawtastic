        ...
        <div class="max-w-7xl mx-auto sm:px-2 lg:px-4">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('metabot.ads.update', ['id' => $ad->id]) }}">
                        @csrf

                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-700">Nombre (referencia interna)</label>
                            <input type="text" name="name" id="name" maxlength="150" value="{{ old('name', $ad->name) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        </div>

                        <div class="mt-3">
                            <label for="source_id" class="block text-sm font-medium text-gray-700">source_id del anuncio</label>
                            <input type="text" name="source_id" id="source_id" maxlength="128" value="{{ old('source_id', $ad->source_id) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        ...
                        <div class="mt-3">
                            ...
                            <select id="scope" name="scope" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                <option value="product_set" {{ old('scope', $ad->scope) === 'product_set' ? 'selected' : '' }}>Set de productos</option>
                                <option value="site_wide" {{ old('scope', $ad->scope) === 'site_wide' ? 'selected' : '' }}>Todo el catálogo</option>
                            </select>
                        </div>
                        ...
                        <div class="mt-3">
                            ...
                            <select id="status" name="status" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                <option value="active" {{ old('status', $ad->status) === 'active' ? 'selected' : '' }}>Activo</option>
                                <option value="paused" {{ old('status', $ad->status) === 'paused' ? 'selected' : '' }}>Pausado</option>
                            </select>
                        </div>

                        <div class="mt-3">
                            <label for="welcome_text" class="block text-sm font-medium text-gray-700">Saludo personalizado (opcional)</label>
                            <textarea name="welcome_text" id="welcome_text" rows="2" maxlength="1024" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('welcome_text', $ad->welcome_text) }}</textarea>
                        </div>
